...
inline std::pair<double, double> getPadCenter(int ring, int pad) {
    /*
    ...
    */
    ...
    double r_center = min_radius + ring_width * (ring + 0.5);

    // Determine the angular offset for odd rings
    double theta_offset = (ring % 2 == 0) ? 0.0 : delta_theta / 2.0;

    // Compute the angular center of the pad (in radians)
    double theta_clockwise = pad * delta_theta + theta_offset + delta_theta / 2.0;
